...
export default function SparePartPage() {
  ...
  const load = useCallback(async () => {
    ...
    const d = await fetch(`/api/sparepart?${params}`).then(r=>r.json())
  ...
  const save = async () => {
    ...
    const res = await fetch(url,{method:modal==='edit'?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...form,stok:+form.stok,minStok:+form.minStok,hargaBeli:+form.hargaBeli,hargaJual:+form.hargaJual})})
    setSaving(false)
  ...
  const del = async (id: string) => {
    ...
    await fetch(`/api/sparepart/${id}`,{method:'DELETE'})
  ...
  const tambahStok = async (p: any) => {
    ...
    await fetch(`/api/sparepart/${p.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({stok:p.stok + +n})})
    ...
    <div style={{display:'flex',height:'100vh',overflow:'hidden'}}>
      ...
      <main style={{flex:1,overflow:'auto',padding:20,display:'flex',flexDirection:'column',gap:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <h1 style={{fontSize:18,fontWeight:600}}>Spare Part</h1>
          {role==='ADMIN' && <Btn variant="amber" onClick={()=>{setForm(emptyForm);setModal('add')}}>+ Tambah Part</Btn>}
        </div>
        <div style={{display:'flex',gap:10}}>
          <input style={{flex:1}} placeholder="Cari nama part..." value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==='Enter'&&load()} />
          <select style={{width:140}} value={filterKat} onChange={e=>setFilterKat(e.target.value)}>
        ...
        <div style={{background:'var(--bg2)',borderRadius:12,border:'0.5px solid var(--border)',overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr>{['Nama','Kategori','Satuan','Stok','Min Stok','Harga Beli','Harga Jual','Aksi'].map(h=><th key={h} style={{padding:'9px 12px',textAlign:'left',fontSize:10,color:'var(--t3)',borderBottom:'0.5px solid var(--border)',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
            <tbody>
              {parts.map(p => {
