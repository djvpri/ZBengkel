...
export default function KendaraanPage() {
  ...
  const load = useCallback(async () => {
    ...
    const d = await fetch(`/api/kendaraan?${params}`).then(r=>r.json())
  ...
  const save = async () => {
    ...
    setSaving(true)
    const res = await fetch('/api/kendaraan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...form,tahun:form.tahun?+form.tahun:undefined})})
    setSaving(false)
  ...
  const del = async (id: string) => {
    ...
    await fetch(`/api/kendaraan/${id}`,{method:'DELETE'})
    ...
    <div style={{display:'flex',height:'100vh',overflow:'hidden'}}>
      ...
      <main style={{flex:1,overflow:'auto',padding:20,display:'flex',flexDirection:'column',gap:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <h1 style={{fontSize:18,fontWeight:600}}>Data Kendaraan</h1>
        ...
        <div style={{background:'var(--bg2)',borderRadius:12,border:'0.5px solid var(--border)',overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr>{['No. Plat','Jenis','Merk & Tipe','Tahun','Pemilik','No. HP','Total WO','Aksi'].map(h=><th key={h} style={{padding:'9px 12px',textAlign:'left',fontSize:10,color:'var(--t3)',borderBottom:'0.5px solid var(--border)',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
            <tbody>
              {kends.map(k=>(
